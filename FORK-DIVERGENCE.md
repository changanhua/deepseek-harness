...
| 20 | `.github/workflows/ci-upstream-watch.yml`, `scripts/generate-divergence-report.ts` | A weekly and manual read-only canary compares the official head with the recorded observation before installing dependencies. A new head produces a synthetic-merge report, affected packages, core-patch heatmap, a blocking hosted-Windows gate, and a non-blocking hosted-Linux advisory; it never updates the baseline, merges, pushes, or modifies `master`. |
