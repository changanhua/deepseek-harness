...
`VerificationWorkspaceLease.assertUnchanged()` rechecks the target's repository identity, HEAD, index, and tracked inputs before and after each verification check, while permitting untracked outputs. Drift or an unprovable inspection prevents a Verdict and preserves the checkout; Queue records verification failure and ordinary acceptance remains unavailable. These observations are not an OS sandbox or continuous filesystem monitoring; the [local provider](../../packages/delivery/repo-workspace-git-local/README.md#known-limitations-and-deferred-work) owns Git-specific limitations.
