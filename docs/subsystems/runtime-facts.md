...
One active owner may register each key. Registration validates resolver exclusivity and relevance names before installing an effect; duplicate ownership fails loud. Disposing the owner removes the declaration and its cache. Static synchronous facts are observed at registration; static asynchronous facts share and retain their first inspection result; dynamic facts resolve on every observation.
