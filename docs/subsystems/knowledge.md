...
The [knowledge package group](../../packages/knowledge/README.md) owns source-grounded, editable libraries. It separates content and business commits from Queue execution. Optional SiYuan projection records keep editable documents connected to the same project without treating a Markdown export as the business ledger. The add-on composition isolates the knowledge Domain into the Web Host storage realm so its repository and Queue bridge resolve one storage owner. The [decision record](../../.agents/notes/implemented/feature/2026-09-08-knowledge-base-native-codex.md) explains that ownership boundary; package READMEs own configuration and user operations.
