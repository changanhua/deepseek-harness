...
两次 tsdown 都接收完整 workspace，且都只打包 `lib/types` 中由对应 tsc 阶段发射的 JavaScript。根配置不扫描 Client 产物、不按包名分类，也不向 tsdown 传维护式 filter；其 Host 默认入口为 `lib/types/{index,invariant,startup}.js`，各包的本地配置则根据 `DSH_BUILD_FACE` 返回不同入口。普通 Client 插件在 Client 阶段一起生成 Node loader 入口与 browser bundle。
