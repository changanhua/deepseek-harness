...
状态：修订后的实施设计草案，尚未冻结验收。WP0–WP4 可按工作包推进；WP5 补齐具体模型、样本清单和用户费用上限并提交后，才可启动 WP6/WP7。本文描述拟实现行为，不代表这些机制已经通过运行验收。
...
| Browser 调用额度 | 每任务进入 Host Browser.execute 的操作总计 ≤40 次，包含 browser 工具、动态插件内部操作、自动快照与失败重试；同一次执行仅计一次。安全收尾单独计数且不得因预算阻止，不能使超预算任务转为成功 |
...
| 真实模型预算 | 模型组为 Codex 基线、v4-flash；每模型每正向 fixture 和每模型拒绝 fixture 恰好 3 次全新会话。具体 model/provider、运行参数及用户费用上限在冻结提交填入，不以展示名代替；新增模型必须先升级口径版本 |
