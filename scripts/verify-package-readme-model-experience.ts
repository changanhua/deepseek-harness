...
const SENTENCE_MODEL_EXPERIENCE: Readonly<Record<string, SentenceContract>> = {
  ...
  'packages/task-queue/task-queue': { kind: 'indirect', reason: 'The contract service registers no model interface; dsh-tool-task-queue owns the model-facing tools and prompt section.' },
  'packages/task-queue/task-queue-local': { kind: 'indirect', reason: 'The durable backend registers no model interface; dsh-tool-task-queue owns the model-facing tools and prompt section.' },
  ...
  'packages/task-queue/tool-task-queue': { kind: 'indirect', reason: 'The toolkit registers the task_queue_* tools, prompt section, and notification hooks documented in its README.' },
  'packages/task-queue/command-task-queue': { kind: 'none', reason: 'The human-facing /queue command renders records directly and registers no model interface.' },
  'packages/task-queue/task-queue-remote': { kind: 'none', reason: 'The browser wire interface renders durable records; dsh-tool-task-queue owns the model-facing tools.' },
  ...
  'packages/client/ui-capability': { kind: 'none', reason: 'Browser-side capability projection; loading a snapshot reads bounded host registries and registers no model interface.' },
  ...
  'packages/client/ui-settings-skills': { kind: 'none', reason: 'Browser-side read-only skills management UI; its snapshot projection registers no model interface.' },
