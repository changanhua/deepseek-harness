...
The personal scope is `@changanhua`. Each confirmed personal package records its current official-shaped name, unique target name, and publication policy. All entries use `blocked-until-rescoped`; the source package names, imports, bundle rows, TypeScript paths, lockfile, and release families remain unchanged in this decision.

## Publication firewall

[`scripts/package-identities.ts`](../../../../scripts/package-identities.ts) requires a GitHub Actions context and rejects an `@deepseek-ai/*` publication unless `GITHUB_REPOSITORY` is `deepseek-ai/deepseek-harness`. It rejects an `@changanhua/*` publication unless the repository is `changanhua/deepseek-harness`, the target name is explicitly registered, and its policy is `personal`. Missing Actions or repository identity and unowned scopes fail closed.

The dsh, vendor, baseline, and Landlock publication paths run the check before their first registry request. Tag-only release verification uses `RELEASE_VERIFY_TAG` and does not enter the registry-write guard, so documentation deployment can validate a release tag without npm authority. The official npm release workflows guard every job by repository identity, and the public Python publication jobs carry the same official-repository restriction until the personal distribution owns separate PyPI names. A personal-fork dispatch cannot reach token-bearing jobs. These checks prevent accidental use of repository release tooling; npm or PyPI credentials and trusted-publisher configuration remain the external authorization boundary, and a hostile local process can bypass repository scripts by calling a registry client directly.
...
A package becomes publishable only when its complete module-resolution and configuration closure uses its target name, its repository metadata points to the personal repository, and the registry policy changes to `personal`. Personal packages may continue to depend on unchanged official Service Definitions. A package that requires a privately modified upstream implementation remains source-only until that dependency is extracted or receives its own personal release identity.
...
The fork has a personal namespace without claiming that a personal npm release exists. Repository publication paths fail before registry access when their Actions and repository identities disagree, and each personal package has one reviewable target identity and an explicit stop state.

The source tree remains mixed until the separate rescope changes land. That costs an additional registry and guard, but it keeps the first change reversible and makes each later package-family migration prove its dependency closure before publication becomes possible. The earlier [release-sequence](2026-08-10-npm-release-sequences.md) and [access-level](2026-08-13-public-vendor-and-native-sequences.md) decisions still govern official-scope artifacts; this decision adds repository ownership and the personal distribution boundary.
