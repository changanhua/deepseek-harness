...
The MCP server exposes fixed operations for runtime identity and current Loader or Session-scoped capability inspection; Session open, prompt, cancel, wait, events, live observation and question answers; source-free Cordis inventory; browser instances, tabs, snapshots and entry inspection; write reconciliation; and evidence export. Registry inspection queries the existing projection owners and bounds filtered results. Optional browser, Cordis and inventory services are resolved for each operation: Loader activation order cannot freeze absence, and unloaded providers cannot remain cached. Provider changes leave the adapter's run binding and receipts intact. It exposes no arbitrary Remote endpoint, Context object, Node.js execution, shell, filesystem, Dynamic Cordis mutation, or acceptance decision.
...
Source tests cover run and Session binding, bounded idempotency, waiting, Cordis filtering, browser observation fences, authentication, MCP mapping, and lifecycle disposal. Built-profile and source-profile tests exercise the CLI entry; source shutdown checks assert child exit and temporary-home removal after both close and disconnect. A Codex app-server probe against the browser page-model worktree has driven a real model question, supplied a canary answer, observed its return in an assistant message and exported evidence. Browser-visible acceptance remains owned by the browser page-model integration. Per-file unit coverage remains below the repository's 100% threshold.
