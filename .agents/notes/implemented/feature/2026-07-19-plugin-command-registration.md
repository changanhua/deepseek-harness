...
The Web command directory coalesces notifications received during an active pull. Success permits one follow-up; failure reaches waiting callers and requires explicit warmup, submission, preset change, or reconnection to retry. Restoring an Agent can register commands before its preset fails, then remove them during rollback. Retrying every resulting notification would recursively restore that same failing Agent and exhaust browser requests. Failed entries therefore remain failed under ordinary catalog-change notifications, even when an unrelated command changes.
