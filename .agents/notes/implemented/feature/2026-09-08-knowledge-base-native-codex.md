...
The personal [knowledge package family](../../../../packages/knowledge/README.md) separates business records, typed Queue execution and user operations. The knowledge Domain owns approved specifications, immutable source versions, stage inputs, captured responses and release records. Queue owns Work/Attempt state. Source snapshots, version exports, and immutable artifacts retain execution evidence. An optional SiYuan projection owns editable document mappings, observed baselines, and create intents. The add-on patch composes these services over existing subprocess capabilities, places the knowledge Domain in the Web Host storage realm, and lets a trusted Profile select the native Codex permission mode including read-only execution.

Stages freeze the prompt and input identity before admission. The Queue bridge binds the Work before invoking the supported `@deepseek-ai/dsh-subagent-codex/app-server-run` boundary. It captures a returned response before cleanup, then accepts content only after cleanup completes. An explicit recovery operation verifies the captured response or publishing candidate; it does not manufacture a response for an unknown remote call. Native inherited model identity and usage remain unknown when the adapter does not expose them.
