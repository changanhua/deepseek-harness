...
Web 命令目录会合并拉取期间收到的通知。成功后允许补拉一次；失败则交付给等待方，仅由显式预热、提交、预设切换或重连重试。恢复 Agent 时，预设失败前可能已经注册命令，回滚时再移除这些命令。如果每条通知都触发重试，就会递归恢复同一个失败的 Agent，耗尽浏览器请求资源。因此，普通目录变更通知不会重试失败项，即使变更来自无关命令也是如此。
