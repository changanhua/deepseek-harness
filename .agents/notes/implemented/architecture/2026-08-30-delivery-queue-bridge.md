...
Admission resolves strict Protocol records and persists only immutable execution facts. The code-change policy digest covers executor identity, optional model, native permission mode, explicit environment, disposal grace, and model-output limit. Queue separately persists the resource claim and retry ceiling. Verification derives its target and plan from the Packet and independently proves exact base-to-target ancestry.

Preparation requires the requested Attempt to be the Work's active starting Attempt, cross-checks matching operator `list()` and `get()` views, parses the kind-specific resolved schema, and compares every persisted resolved fact with the prepared admission. Verification applies the same exact validator to the selected bound change's binding, Work state, Result, successful Attempt, resolved facts, and completion claim. Preparation then materializes provider proofs plus operation-local closures. It may inspect revisions and bind evidence provenance, but it does not open a checkout, spawn a process, or publish evidence. `start()` calls the runner or verifier and synchronously returns its live cancellation and settlement owner.
...
Activation obtains operator authority only inside the trusted Host composition. It validates the Delivery snapshot, cross-checks exact Queue `list()` and `get()` views, and reconciles every `submitting` binding before registering either pump-capable handler. Recovery accepts only an exactly reconstructed canonical input and deterministic key. A failed activation cannot start a runner or verifier, and rollback or normal disposal attempts every registered disposer before reporting collected failures. Recovery stores no projection and never creates an acceptance decision.
...
Delivery work has one Queue lifecycle and one Delivery binding lifecycle. Handler disposal removes both registrations, restart converges incomplete handshakes through Queue idempotency, and no runtime path automatically accepts a delivery.
