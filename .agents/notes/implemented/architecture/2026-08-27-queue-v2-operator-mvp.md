...
**Unscoped Batch controls.** Rejected because an action over every visible task hides which Batch is affected and whether running work may have side effects. The workbench focuses one Batch and requires the normal risk acknowledgement before canceling unfinished members.

## Consequences

The workbench is small enough for routine task management while preserving the durable Queue v2 model. Artifact browsing, server pagination, and structured success confirmation remain deferred. A future LAN operator surface must add authentication before re-enabling a non-loopback bind.
