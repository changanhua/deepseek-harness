...
Queue v2 Remote 把 durable 记录投影为四种 operator 状态：queued、running、attention 和 done。终态 done 行另带 succeeded、failed 或 canceled outcome；durable 生命周期事件和恢复语义保持不变。四状态投影刻意小于 durable 状态词汇表；operator 紧急度排序是客户端投影，绝不是持久化的优先级。[Queue operator 诊断与 Batch 操作](2026-09-07-queue-operator-diagnostics-and-batch-actions.zh.md)持有运行时等待解释与 Batch 聚焦扩展。
...
store 在刷新失败时保留最后一次成功的行、详情与刷新时间戳，并如实标注。待定的 mutation 只锁定其自身 work ID，因此搜索、筛选、选中、刷新与无关行仍可用。工作台复用共享 primitives 中的 `RiskConfirmation`、`Toast` 与 `JsonTree`，而不是包内自制等价物。分析、任务创建与结构化成功确认保持缺失。
...
**无范围约束的 Batch 控件。** 拒绝，因为作用于全部可见任务的操作会隐藏受影响的 Batch，也无法明确运行中 work 是否可能已有副作用。工作台先聚焦一个 Batch，并在取消未完成成员前要求正常的风险确认。

## Consequences

工作台足以处理日常任务管理，同时保留 durable Queue v2 模型。artifact 浏览、服务端分页与结构化成功确认仍延后。未来 LAN operator surface 必须先增加认证，才能重新启用非 loopback 绑定。
