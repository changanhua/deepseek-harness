...
当前有两个能力缺口阻止实施。可信 Queue operator facade 可以检查和修改 work，却无法准入 ownerless work，尽管持久模型支持 operator receipt 和 `ownerSessionId: null`。Codex provider 从真实 parent Session 获取 cwd，而 Delivery 必须在 Attempt 拥有的 worktree 中运行，不能虚构 supervisor Session。
...
第一版实现位于 fork 的 `packages/delivery/` group，因为它依赖 fork 自有的 Queue v2 约定。Bundle 只包含 composition。Queue core 不导入 Delivery、GitHub、Git workspace、verifier、executor、Remote 或 UI package。Intake 调用 Delivery 而非 Queue；Bridge 同时消费 Delivery 和 Queue，并注册 Delivery WorkKind。
...
Bridge 提出 provider-neutral 的 `code.change@1` 和 `code.verify@1` WorkKind。Change work 的 Queue success 只记录 typed completion claim。验证作为独立 work 对精确 checkpoint commit 运行。只有匹配的 passed verdict 才允许普通验收；显式人工 waiver 记录例外。
...
**在 Codex proof 之前创建 generic executor registry。** 否决，因为一个 provider 和一个 consumer 不足以证明 public capability 的必要性。Gate B 先验证真实 lifecycle，并揭示最小稳定可复用输入。
