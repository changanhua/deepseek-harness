...
Gate A extends trusted operator admission without exposing generic enqueue to the browser. Ownerless admission uses operator authority, operator-scoped idempotency, `ownerSessionId: null`, and no Session Notification. The same Queue change closes the post-start ownership gap: if the running append fails after `LiveAttempt` exists, the provider aborts its controller, requests `LiveAttempt.cancel()`, observes cancellation and `live.done` within the configured bound, and only then records `unknown` plus Attention. Cancellation rejection, deadline, a conflicting late outcome, or another persistence failure remains post-start evidence; Queue never reclassifies it as `not-started`, retains root ownership until a durable terminal or unknown record represents the execution, and never auto-retries it.

Gate B proves real Codex execution in a disposable explicit worktree, cancellation propagation, whole-process-tree quiescence, and truthful terminal classification. The current `startCodexRun` still accepts parent-bearing `SubagentStartRequest`, although its lower-level run specification accepts explicit cwd. The gate therefore extracts a parent-free, non-Subagent contract through the package's supported root entry while preserving the Session-backed provider adapter; Delivery may not deep-import source-only files. A governed `codex exec --json` subprocess adapter remains a fallback only if the executable proof exposes a cancellation or quiescence defect.
...
- Gate A admits ownerless work idempotently and prevents an unowned live process after a post-start durability failure.
