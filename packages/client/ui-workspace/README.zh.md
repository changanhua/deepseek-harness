...
工作台从**总览**打开，提供**需要我处理**、**正在进行**、**最近工作**、项目概览以及**能力与工具**。这些页面读取已有的 Workspace、Session 和待交互投影；查看项目不会创建或选择 Session。工作列表排除空白、已归档和子代理来源的 Session，运行中的后代通过所属父会话呈现。**有新结果**只表示会话在未读时结束，不代表输出已通过验证或验收。打开工作会进入已有对话；工具卡片只打开当前组合已经注册的模块。
