...
每个项目页都有**项目资源**区。通过**添加资源**写入 Markdown 资料、登记已有项目文件，或保存前台服务命令。**打开内容**显示有界的纯文本；原文件通过普通文件工具编辑。服务行显示命令、进程状态、最近日志和手动启停操作。**刷新状态**读取当前观察结果；显示状态不是持续健康监测。**移除登记**会保留原文件，并要求先停止运行中的服务。保存位置折叠区提供 Agent 可读取的配置路径。[Workspace Controller](../../api/workspace-controller/README.zh.md#project-resources) 负责持久化和服务生命周期。

工作台从**总览**打开，提供**需要我处理**、**正在进行**、**最近工作**、项目概览以及**能力与工具**。这些页面读取已有的 Workspace、Session 和待交互投影。总览和项目页嵌入已有的 Conversation 输入组件，包含工作区、预设、附件、权限和模型控件，并通过正常的新会话流程准备可复用的空白 Session。第一条消息发送后进入对话，无需经过第二个输入页面。工作列表排除空白、已归档和子代理来源的 Session，运行中的后代通过所属父会话呈现。**有新结果**只表示会话在未读时结束，不代表输出已通过验证或验收。打开工作会进入已有对话；工具卡片只打开当前组合已经注册的模块。
