...
Each project page has a **Project resources** section. Use **Add resource** to write a Markdown note, register an existing project file, or save a foreground service command. **Open content** displays bounded plain text; edit the original file with normal file tools. Service rows show the command, process state, recent logs, and manual start/stop actions. **Refresh status** reads current observations; the displayed state is not a continuous health monitor. **Remove reference** keeps the original file and requires a running service to be stopped first. The storage-location disclosure gives the Agent-readable configuration path. [Workspace Controller](../../api/workspace-controller/README.md#project-resources) owns persistence and service lifetimes.
