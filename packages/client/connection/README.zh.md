...
随附的 `dsh-web-app` Profile 关闭浏览器认证，因此 loopback index、Host RPC 与 WebSocket 请求使用干净的根 URL，不需要启动 token 或 cookie。需要进程 token 与签名浏览器会话时，在 Profile patch 中把 Connection `browserAuth` 设为 `true`。无论哪种模式，静态资源都保持公开，HTTP 载体也不接受 Authorization header token。

启用 `browserAuth` 时，每个进程生成一个随机启动令牌。`dsh-web-app` 打印并打开带 `?token=...` 的根 URL；`frontend-static` 把根路径和 index 请求交给 `ctx.connection.authorizeIndex`，后者只在 `GET /` 接受该令牌，写入绑定 authority 的签名 cookie，再重定向到干净的 `/`。cookie 签名密钥是 `ctx.credentials` 中由 `client-connection/browser-session` 拥有的 grant 记录，`cookieMaxAgeDays` 默认设为 30 天。

每个请求都会经过 `src/api-request-trust.ts`。其 `Host` 必须是 loopback，或与 `trustedHosts` 条目匹配：带端口的 `host:port` 精确匹配，不带端口的条目匹配任意端口，两侧均经 WHATWG 归一化。若附带 `Origin`，它必须等于该 Host；`sec-fetch-site: cross-site` 一律拒绝。畸形配置 authority 会让插件加载失败。这些检查防御 DNS rebinding 与跨站浏览器请求，绝不建立身份。Host/Origin 校验失败返回 403；启用浏览器认证时，Host 可信但未认证的请求返回 401。`dsh web --host 0.0.0.0` 仍不受支持。
