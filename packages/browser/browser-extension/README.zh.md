...
树中的元素节点还会公开一组许可属性、边界和视口归属，使使用方无需接收任意页面数据也能定位内容区域。只读 `entry_inspect` 操作要求 `regionSelector` 只匹配一个区域，并要求 `selector` 以 `:scope` 开头；它在修改页面之前返回绑定质量和有界样本。限定区域的 `entry_mount` 对初始条目和后续变更使用同一边界，因此文档其他位置的匹配元素不会获得挂载控件。
