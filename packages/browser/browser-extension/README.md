...
The caller supplies the request id before any dispatch, and the Host journals it before the worker receives the action. The same id returns its retained receipt; `requestStatus()` requires read authority, returns a detached value, and never replays the request. A response with `in-flight` or `unknown` remains a recovery fact, not a retry permission. A grant epoch rollover may query status or clear a region owned by an older epoch, but it cannot issue a new write against that old ownership.

`page_map` records short-lived page evidence keyed by Session, installation, epoch, and exact page identity. `region_render` must use that evidence. Append creates an extension-owned panel; replace is admitted only for a mapped disposable, non-protected region. The Host reserves region capacity before dispatch and keeps that reservation for an unknown result, so concurrent calls cannot oversubscribe a page. `region_clear` confirms release only when the worker observes `cleared:true` or proves the exact mount is already absent; a sent `document_replaced` records a vanished resource, whereas `target_url_stale` leaves it unresolved. Host registrations advance a generation for every render or mount, so a retained late settlement cannot remove a later reuse of the same id; if the newer operation deterministically fails, an earlier observed cleanup remains authoritative. The worker's page runtime tracks mounts without placing mount ids in CSS selectors, and a document change never silently binds a mount to a new URL. Internal snapshot presentation queries return a positive fact only for a matching live runtime-owned panel, not for identical text elsewhere on the page.
