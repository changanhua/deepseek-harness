...
`page_map` records short-lived page evidence keyed by Session, installation, epoch, and exact page identity. `region_render` must use that evidence. Append creates an extension-owned panel; replace is admitted only for a mapped disposable, non-protected region. The Host reserves region capacity before dispatch and keeps that reservation for an unknown result, so concurrent calls cannot oversubscribe a page. `region_clear` confirms release only when the worker observes the clear; `document_replaced` records a vanished resource, whereas `target_url_stale` leaves it unresolved. The worker's page runtime tracks mounts without placing mount ids in CSS selectors, and a document change never silently binds a mount to a new URL.
