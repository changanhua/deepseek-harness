...
已交付的浏览器功能可由其已认证安装跨对话查看。只有活跃运行拥有浏览器挂载，或其 Client 半正在运行且没有已记录的渲染失败时，检查结果才包含 `openTarget`。打开只定位现有挂载或其 Web 会话，不启动运行。没有可视界面的纯 Host 功能和已停止功能没有打开目标。扩展在定位页面资源前检查当前包、运行身份及精确文档。
