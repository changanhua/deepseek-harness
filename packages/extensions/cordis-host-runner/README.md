...
A host half can use the managed `harness.browser` facade to map a target page, render a high-level presentation into one opaque region reference, and restore its exact mount. The runner captures the exact live defining Agent; generated code cannot supply a Session identity, reset the BrowserTask budget, inspect the private selector, or bypass Provider recovery policy. Every ordinary call and stop, update, undefine, startup-failure, or owner-scope cleanup runs under that Agent initiator. All local mount input is validated before ownership is registered. Owner-scope cleanup begins before the Agent detaches; if the owning BrowserTask is already terminal, only the runner's previously recorded exact cleanup transfers to the runner ledger instead of reopening that task. If a cleanup was sent but remains unknown, the orphan path polls only that original request identity and never executes it again. A later permanent `forgetCollected` request remains distinct from an earlier ordinary unmount.
