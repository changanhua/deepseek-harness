...
export const HOST_BUILTIN_INSPECTION = [
  ...
  },
  {
    ...
    signatures: [
      ...
      'harness.browser.render(input: { installationId: string; page: { tabId: number; frameId: number; documentId: string; url: string }; slot: string; regionRef: BrowserRegionRef; mode?: append | replace; placement?: prepend | append; presentation: { title?: string; summary?: string; items?: Array<{ title: string; meta?: string; link?: string }>; facts?: Array<{ label: string; value: string }>; links?: Array<{ text: string; href: string }>; footer?: string } }, signal?: AbortSignal): Promise<BrowserActionResult>',
