...
对于 Codex 的纯文本协助，在 base Bundle 之后应用可选的[只读 Host patch](examples/codex-read-only.patch.yml)。它通过独立命名的提供方暴露 `subagent_codex_readonly`，将原生沙箱固定为只读，并在发起调用的 Session 中等待结果。Codex 使用其已有登录和默认模型。此 patch 为该 Host 增加一个工具，不修改其他 Codex 提供方，也不启用后台回调。
...
派发前，工具记录原生调用身份、配置的提供方、实际 prompt 摘要和父级输入修订。父级已有待消费输入时，不启动新的前台委派。运行期间插入的新输入会使成功结果与部分输出都过期，即使该输入随后被移除。过期原文保留在不进入模型历史的 Session 日志事件中；模型收到错误，要求重新考虑当前请求。其他 Session 事件不会使委派过期。资源释放后，工具再次检查取消信号，随后才能返回完成结果。

此绑定仅覆盖当前存活的前台调用。它不创建异步外部回调，不在 Host 重启后重放子任务，也不将 prompt 相同的独立工具调用视为重复操作。原始工具调用与结果仍在父级 Session 日志中关联。
