...
Safe automatic retries remain queued until their eligibility time. The local provider uses exponential delays of 1, 2, 4, 8, 16, then at most 30 seconds, derived from the persisted automatic-retry authorization time (`WorkState.updatedAt`) and completed attempt count. Only automatic authorization retains a retriable `not-started` failure on queued work; manual and unknown-resolution retries clear it. Log replay and snapshot recovery therefore reconstruct the same deadline without changing schema 3. Waiting retries reserve no execution or resource slots. One disposable timer wakes the dispatcher and keeps an otherwise idle host alive; pause and shutdown clear it, and resume or handler registration reconstructs it. The operator wait reason is `retry-backoff` with an ISO `eligibleAt`; capacity may delay dispatch further. Running work is never restarted merely because this deadline passes.

Handlers declare `ResourceClaim`s, and admission rejects claims missing from deployment `resourceCapacity`. `maxConcurrent`, persisted resource claims, and a Batch's `maxParallel` jointly bound dispatch; unused host capacity remains available to other eligible Batches. The operator facade reports provider dispatch as running, paused, or faulted, plus one current wait reason for queued work: paused dispatch, store fault, unavailable handler, global capacity, Batch capacity, resource capacity, or a short scheduler turn. `pause()` stops all new dispatch globally: read, admission, cancellation, acknowledgement, and restricted unknown resolution remain available.
