...
`LocalTaskQueue` acquires an `owner.lock` on the queue root before it reads the durable log or reclaims crashed tasks. The lock is created through an atomic `link(2)` from a fully-written temporary file, so a second host process on the same `queueRoot` is refused before it can `recover()` the first host's live tasks. A lock whose recorded pid is dead (the previous host crashed) is archived into `quarantine/` and taken over; a lock held by a live pid, by a different host machine, or by the same process refuses startup. A shared queue root across machines is not supported. On teardown the lock is released (best-effort); a leftover file is recovered by the stale-takeover path on the next acquire.
