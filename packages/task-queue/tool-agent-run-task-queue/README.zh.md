...
`@changanhua/dsh-tool-agent-run-task-queue` 提供 `agent.run@1` 的 WorkKind 专用 Queue 准入。它从实时 Agent Session 派生 owner 权限，不暴露 executor、profile、model、credential 或 shell 选择。
...
Queue provider 解析并持久化 worker 规范、策略与资源声明。通用列表、状态、结果、取消、重试、统计和 Notification 投递仍由 `@changanhua/dsh-tool-task-queue` 提供。
