...
Before each check and after its process tree and output publication settle, the verifier awaits the lease's `assertUnchanged()`. A changed HEAD, index, tracked input, or failed integrity inspection rejects `done` with `workspace-integrity`, stops later checks, and preserves the checkout for operator inspection. The Queue bridge records a failed verification, so ordinary acceptance has no passed verdict to consume. Untracked build outputs are allowed; checks that rewrite tracked generated files are not unchanged-target verification.
...
- **Integrity is observed between checks** — edits restored within a single check, Git clean-filter equivalence, and same-user concurrent writes require stronger deployment isolation. An execution or output-publication error already prevents a verdict but can bypass the post-check inspection and retain the ordinary quiescent cleanup behavior.
