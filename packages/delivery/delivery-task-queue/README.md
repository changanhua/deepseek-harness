...
Both handlers persist strict Packet, Contract, repository, target, executor, and policy facts during admission. Preparation requires the requested Attempt to be the Work's active starting Attempt and compares every parsed resolved fact against the prepared admission. Verification also validates the selected bound change's exact binding, Work state, Result, successful Attempt, resolved facts, and completion claim. Preparation then materializes only provider proofs and operation-local closures, and performs no checkout or process side effect. `start()` synchronously returns Queue live ownership. Cancellation propagates to the runner or verifier; validation and startup failures settle `failed/not-started`, quiescent product or completion failures settle `failed/started`, and ownership or cleanup uncertainty settles `unknown/unknown`. Every failure is non-retriable under the configured one-Attempt default.
