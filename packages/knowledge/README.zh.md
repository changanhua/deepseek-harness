...
此包组让 DSH Profile 从有版本的来源构建可编辑知识库、检查结果，并在用户选择时发布通过检查的版本。业务包拥有 Domain 记录、工作 Markdown 和发布物；Queue 桥接拥有持久的 Codex 阶段执行；bundle 提供工具和 `/knowledge` 命令。同一受信 Profile 内的项目由其会话共享，本包组不提供 session 私有 ACL。配置、失败处理和模型可见细节请查看各包页面。
...
| Package | Role |
|---|---|
| [`knowledge-base`](knowledge-base/README.zh.md) | 保存项目事实、可编辑条目、不可变产物、检查和发布物。 |
