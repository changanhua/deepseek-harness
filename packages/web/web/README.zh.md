...
`searchProvider` 与 `fetchProvider` 偏好可通过 `web` settings namespace 由用户编辑，并叠加在 composition 配置和同名环境变量覆盖之上。`WebRuntime` 在每次 `search()` 或 `fetch()` 调用时解析该 section，因此编辑无需重启或重新注册提供方即可影响下一次调用；进行中的调用保留其初始偏好。未挂载 settings provider 的部署使用 composition 值。
