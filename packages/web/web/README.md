...
The `searchProvider` and `fetchProvider` preferences are user-editable through the `web` settings namespace and layer over the composition config and matching environment override. `WebRuntime` resolves the section for each `search()` or `fetch()` call, so an edit affects the next call without restarting or re-registering providers; an in-flight call keeps its initial preference. A deployment without a settings provider uses the composition values.
