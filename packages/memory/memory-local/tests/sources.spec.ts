...
import { persistSession } from './harness.ts'
...
describe('memory source provenance', () => {
  ...
  it('propagates cancellation after a physical Session read finishes', async () => {
    ...
    const open = access.persistence.open.bind(access.persistence)
    vi.spyOn(access.persistence, 'open').mockImplementationOnce(async (id, mode, options) => {
      const handle = await open(id, mode, options)
  ...
  it.each(['id', 'cwd', 'missing-cwd'])('rejects persisted Session metadata differing from the authorized query: %s', async (field) => {
    ...
    await persistSession(ctx, session)
    const open = access.persistence.open.bind(access.persistence)
    vi.spyOn(access.persistence, 'open').mockImplementation(async (id, mode, options) => {
      const handle = await open(id, mode, options)
      ...
      return { ...handle, header: meta, read: handle.read.bind(handle), close: handle.close.bind(handle) } as typeof handle
    })
    await expect(captureMemorySources(access, [{ kind: 'session-event', sessionId: session.id, seq: event.seq }], MAX_BYTES))
      .rejects.toMatchObject({ code: 'source-unavailable' })
  })
