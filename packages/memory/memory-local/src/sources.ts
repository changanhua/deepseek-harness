...
async function observe(access: MemorySourceAccess, input: MemorySourceInput, maxBytes: number, signal?: AbortSignal): Promise<Observation> {
  ...
  try {
    ...
    const handle = await access.persistence.open(sessionId, 'read', signal === undefined ? undefined : { signal })
    let stored: SessionInspection | undefined
    try {
      const read = await handle.read(0, undefined, signal === undefined ? undefined : { signal })
      stored = { meta: handle.header, inheritedEventCount: handle.inheritedEventCount, events: read.events }
    } finally {
      await handle.close()
    }
    if (stored === undefined) throw new Error('session persistence returned no inspection')
