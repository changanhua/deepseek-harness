...
Patch 按依赖顺序挂载 `delivery-local`、`delivery-evidence-local`、`repo-workspace-git-local`、`delivery-task-queue`、`delivery-remote` 与 `ui-delivery`。Storage Domain、Subprocess、Queue capacity、transport 和 Web shell 继续由 base layer 持有。
...
- **仅一个启动目录仓库** — P0 profile 只暴露 repository id `workspace`；请从 Issue 指向的精确 Git toplevel 启动 DSH。
