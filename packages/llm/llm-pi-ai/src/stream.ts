...
function classifyPiAiError(message: string): string {
  ...
  // Responses can report `network_error` as the incomplete reason; `_` is a
  // word character, so matching the standalone word `network` misses it.
  if (/\b(?:network(?:_error)?|connection|socket|fetch)\b|\bECONN[A-Z]+\b/i.test(message)
