...
`opencode-go` 路由按 [OpenCode Go 的路由要求](https://opencode.ai/docs/go/#where-can-i-use-it)，通过 `x-opencode-session` 发送 Harness Session ID。该值在同一对话中保持稳定，并覆盖 profile 中名称大小写不同的冲突标头。没有 Session 的调用会获得新的路由 ID；其他提供方不会被额外添加 OpenCode 标头。
