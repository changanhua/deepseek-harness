...
The skill projection mirrors `ctx.skills.managementSnapshot()` for the viewing session's scope, including each candidate's name, description, invocation policy, source, provider, path, origin, selection state, shadow provenance, and JSON-safe diagnostics. The management Remote returns that full read-only projection with every mutation action disabled. The tool projection mirrors `ctx.tools.schemas()`, tagging MCP-bridged tools (`mcp__<serverName>__<rawName>`) with their server and raw MCP name. The MCP server projection reads Loader entries whose module is `@deepseek-ai/dsh-mcp-client`, exposing only the configured `serverName`, `transport`, and the count of tools currently registered under that server's namespace.
