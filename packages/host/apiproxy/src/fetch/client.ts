...
import { randomUuid } from '../api/uuid.ts'
...
import { skillManagementSnapshotValueSchema } from '../api/skill-management.schema.ts'
import {
...
export interface IApiClient {
  ...
  skillManagement: {
    snapshot(payload: RequestPayload<'skillManagement.snapshot'>, signal?: AbortSignal): Promise<RpcResponse<ResponseValue<'skillManagement.snapshot'>>>
  }
  agentPresets: {
...
const UNARY_VALUE_SCHEMAS: { [K in keyof RpcMethodMap]: z.ZodType<Wire<ResponseValue<K>>> } = {
  ...
  'skillManagement.snapshot': skillManagementSnapshotValueSchema,
...
export abstract class AbstractApiClient implements IApiClient {
  ...
  protected mintRpcId(): RpcId {
    // randomUuid backs off to crypto.getRandomValues, which browsers expose on
    // insecure origins; crypto.randomUUID is secure-context-only and absent on
    // plain-HTTP LAN pages (http://<lan-ip>:port), where any call throws.
  ...
  readonly skillManagement: IApiClient['skillManagement'] = {
    snapshot: (payload, signal) => this.callUnary('skillManagement.snapshot', payload, signal),
  }
