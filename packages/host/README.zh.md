...
`host/` 组提供 Web GUI 的普通 HTTP 服务器、服务已构建 Web 壳的 SPA dist 服务器、带原生／浏览／自适应组合包的工作区目录选择 seam、只读运行时投影，以及本地 Work Observatory 证据。这些都是产品包；浏览器传输位于 [`client/`](../client/README.zh.md)，组合应用是 [`apps/cli`](../../apps/cli/README.zh.md)，它启动 [`dsh-base` 组合包](../bundle/base/cordis.patch.yml) 来提供 `apps/web/` 下的 Web 应用。选择器后端可在共享 seam 后互相替换。
...
下列包分别承担 Host 角色；各包的 README 拥有自己的约定与配置。
...
| [`work-observatory/`](work-observatory/README.zh.md) | 基于 Host 时钟持久保存浏览器活动与 Session step 墙钟时间证据 | `ctx.workObservatory` |
...
- [Work Observatory 子系统](../../docs/subsystems/work-observatory.zh.md)——浏览器与 Session 证据语义、边界及面向用户的汇总。
